...
- **两页统一**：meta 全程 `alpha = 1`、跟随 name 一起迁移进标题栏（副标题角色）。**不再淡出**——之前「我」页 meta 淡出的行为已随统一取消。
...
| `tabPinTop` | 方法 | `topInset + 68`：tab 贴顶顶边（距标题栏底 12pt）。stickyBar / pinOffset / updateStickyTabs 统一取此值 |
...
### 3.5 一页可见时禁止上滑（2(2)a，`syncScrollInset`）

`wantMax` 精确取到「tab 贴顶」为止（`hr.origin.y - tabPinTop`，**不再多给 +24**）。内容不足一屏时 `naturalMax` 小，
底部 inset 只补到「刚好贴顶」→ 贴顶即最大 offset，无法再上滑进空白。内容够多时 `naturalMax` 远大于 `wantMax`，`bottom=0`，自由滚动（走 Zone② detent）。
