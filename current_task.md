...
- **✅ 已发送文件分页与相册文件语义（2026-08-01，用户真机测试通过；按要求未编译）**：服务端新增 `GET /api/v1/files/sent` 游标分页（默认 50、上限 100），`file_name` 贯穿 WS 发送/持久化/实时投递/离线同步；iOS 用按 uid 隔离的 SQLite 缓存本地优先显示，在线首屏刷新并滚动续页。文件面板“从相册中选择”读取图片/视频原始字节，不压缩转码，统一以 `content_type=file` + 原文件名发送并渲染文件卡片。按用户要求不兼容旧消息/旧数据库，验收前已删除 iOS App 与服务端旧数据库并重建测试账号。
...
- 水滴临界帧修复（2026-08-01，按要求未编译）：根据用户截图定位到接触阈值恰好命中时“遮罩已移除、圆角已清零”产生裸方形帧；详情页与“我”页统一改为阈值帧保持圆形，仅在有效吸附进度后切换水滴遮罩，并用禁用隐式动画的 `CATransaction` 原子更新 frame、圆角及 mask path，消除高频滚动时图层不同步闪动。Telegram 原始矢量资源位于 `Resources/UserAvatarMask.tgs`，其遮罩由专用 `AnimationNode` 渲染；当前工程未引入该渲染引擎。
