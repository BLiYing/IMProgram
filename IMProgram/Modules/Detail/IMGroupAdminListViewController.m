...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = IMLocalized(@"group.role.admin");
...
- (void)reload {
    ...
    [IMHTTPService.sharedService groupInfoWithToken:token convID:self.convID
        ...
        if (!group.canManage) { // 我被撤职 / 群主身份被别处转走：整页只对群主与管理员开放
            [self popWithToast:IMLocalized(@"group.error.not_admin")];
            return;
        }
        self.group = group;
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    if (section == IMAdminSecOwner) { return IMLocalized(@"group.role.owner"); }
    return IMLocalizedFormat(@"group.admin_list.count_title", (long)self.admins.count);
}

- (NSString *)tableView:(UITableView *)tableView titleForFooterInSection:(NSInteger)section {
    if (section != IMAdminSecAdmins) { return nil; }
    if (!self.isOwner) { return IMLocalized(@"group.admin_list.owner_only_note"); }
    return self.admins.count > 0
        ? IMLocalized(@"group.admin_list.footer_can_revoke")
        : IMLocalized(@"group.manage.permission_note");
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!m) { // 空态占位
        UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"c" forIndexPath:indexPath];
        cell.textLabel.text = IMLocalized(@"group.admin_list.empty");
...
- (UISwipeActionsConfiguration *)tableView:(UITableView *)tableView
    ...
    UIContextualAction *revoke = [UIContextualAction contextualActionWithStyle:UIContextualActionStyleDestructive
        title:IMLocalized(@"group.admin_list.revoke_btn") handler:^(UIContextualAction *a, UIView *v, void (^done)(BOOL)) {
        [ws confirmRevoke:m];
...
- (UIContextMenuConfiguration *)tableView:(UITableView *)tableView
    ...
    return [UIContextMenuConfiguration configurationWithIdentifier:nil previewProvider:nil
        ...
        [items addObject:[UIAction actionWithTitle:IMLocalized(@"chat.header.view_profile")
                                             image:[UIImage systemImageNamed:@"person.crop.circle"]
                                        identifier:nil handler:^(UIAction *a) { [ws openPeerDetail:m]; }]];
        if (canRevoke) {
            UIAction *revoke = [UIAction actionWithTitle:IMLocalized(@"group.member_action.revoke_admin")
...
- (void)confirmRevoke:(IMGroupMember *)m {
    ...
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:IMLocalized(@"group.member_action.revoke_admin")
                                                                  message:IMLocalizedFormat(@"group.admin_list.revoke_confirm_message", name)
    ...
    __weak typeof(self) ws = self;
    [alert addAction:[UIAlertAction actionWithTitle:IMLocalized(@"group.admin_list.revoke_btn") style:UIAlertActionStyleDestructive handler:^(UIAlertAction *a) {
        [ws setRole:@"member" forUserID:m.userID successToast:IMLocalized(@"group.admin_list.revoke_done")];
    }]];
...
- (void)addAdmins:(NSArray<NSString *> *)uids host:(nullable UIViewController *)host {
    ...
    NSString *token = IMHTTPService.sharedService.currentToken;
    if (token.length == 0) { [(host ?: self) im_showToast:IMLocalized(@"common.not_logged_in")]; return; }
