...
@implementation IMAlbumCell {
    ...
    // 整组发送失败的红❗（宫格外侧、点击重发整组）由 IMMessageCell 基类持有 `_failBadge`，本类只补约束。
    // 注意与 IMAlbumTileView 自己的 _failBadge 不是一回事：那个是**逐格**中心的 "!"，
    // 表达"这一格失败了"；基类那个在宫格外侧，表达"这条消息发失败了"，两者互补。
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        [self installFailBadgeAnchor:[_failBadge.trailingAnchor constraintEqualToAnchor:_container.leadingAnchor constant:-6]];
        ...
        [NSLayoutConstraint activateConstraints:@[
            ...
            _containerBottom,
            // 红❗：钉在宫格左侧、垂直居中（宽高由 IMFailBadgeView 自持；贴左那条仅失败时激活）。
...
- (void)configureWithMembers:(NSArray<IMMessageModel *> *)members mine:(BOOL)mine host:(NSString *)host
    ...
    BOOL anyFailed = NO;
    BOOL anyResendable = NO;   // 组里只要还有一格能重发，红❗就可点（点击重发所有失败成员）
    for (IMMessageModel *m in members) {
        if (m.note.length > 0 && !noted) { noted = m; }
        if (m.status == IMMessageStatusFailed) { anyFailed = YES; }
        if (IMResendPolicyForMessage(m, mine) != IMResendPolicyNone) { anyResendable = YES; }
    }
    [self applyFailBadgeShows:(mine && anyFailed) tappable:anyResendable];
