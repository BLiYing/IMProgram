...
- (void)createTables {
    [_queue inDatabase:^(FMDatabase *db) {
        ...
        // 群主展示资料：群列表副标题「群主 XXX」离线首屏也要显示名字而不是内部 ID（老库补列）。
        for (NSString *col in @[@"owner_nickname", @"owner_username"]) {
            if ([self column:col existsInTable:@"im_group_local" db:db]) { continue; }
            NSString *sql = [NSString stringWithFormat:
                @"ALTER TABLE im_group_local ADD COLUMN %@ TEXT NOT NULL DEFAULT ''", col];
            if (![db executeUpdate:sql]) {
                IMLogDatabase(@"迁移失败：im_group_local 补列 %@ 未成功: %@", col, db.lastErrorMessage);
            }
        }
    }];
}
