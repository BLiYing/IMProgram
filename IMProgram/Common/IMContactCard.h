...
@property (nonatomic, copy) NSString *userID;               ///< u：**内部 ID**（必有）。点卡片发起聊天/加好友用它，**不得展示**
@property (nonatomic, copy, nullable) NSString *username;   ///< un：公开句柄快照，卡片副标题显 @xxx（老消息无此字段 → nil）
...
/// 无昵称退 `@username`；**绝不回落 userID**（那是 10 位随机数字内部 ID）；
