...
NSString *IMContactCardBuild(NSString *userID, NSString *nickname, NSString *avatarURL) {
...
NSString *IMContactCardPreview(NSString *content) {
    ...
    if (!c) { return @"[个人名片]"; }
    NSString *name = c.nickname.length > 0 ? c.nickname : c.userID;
    return [@"[个人名片] " stringByAppendingString:name];
}
